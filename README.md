...
Si vous avez déjà installé [`assistant-plugins`](https://aymkdn.github.io/assistant-plugins/), et que vous souhaitez ajouter ce plugin, alors :
  - Pour Windows, télécharger [`install_yamaha.bat`](https://github-proxy.kodono.info/?q=https://raw.githubusercontent.com/tapsystem/assistant-yamaha/master/install_yamaha.bat&download=install_yamaha.bat) dans le répertoire `assistant-plugins`, puis l'exécuter en double-cliquant dessus.  
...
#### Les variables "communes":
  ...
  - les paramètres de volumes `int db` et `int by`permettent de donner une indication de volume. Cette donnée doit être passée avec une sa valeur décimale et multipliée par 10. Ainsi pour régler un volume à -45.0 dB, `int db` devra être égale à -450.
...
Il est possible d'exposer d'autres fonctions de paramétrage, de listage, ... voir [`yamaha_simpleCommands.js`](https://github.com/tapsystem/assistant-yamaha/blob/master/yamaha_simpleCommands.js) pour plus de détails (setHDMIOutput, setBassTo, setTrebleTo, setSubwooferTrimTo, setDialogLiftTo, setDialogLevelTo, YPAOVolumeOn, YPAOVolumeOff, extraBassOn, extraBassOff, adaptiveDRCOn, adaptiveDRCOff, selectUSBListItem , selectWebRadioListItem .... )
