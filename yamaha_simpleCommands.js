...
function getInput(to) {	/*TS Addon*/
    if (!to) return "";

    // replace numbers by input name

    switch (to) {
		case 1: to = "TUNER";break;
		case 2: to = "MULTI CH";break;
		case 3: to = "PHONO";break;
		case 4: to = "HDMI1";break;
		case 5: to = "HDMI2";break;
		case 6: to = "HDMI3";break;
		case 7: to = "HDMI4";break;
		case 8: to = "HDMI5";break;
		case 9: to = "HDMI6";break;
		case 10: to = "HDMI7";break;
		case 11: to = "AV1";break;
		case 12: to = "AV2";break;
		case 13: to = "AV3";break;
		case 14: to = "AV4";break;
		case 15: to = "AV5";break;
		case 16: to = "AV6";break;
		case 17: to = "AV7";break;
		case 18: to = "V-AUX";break;
		case 19: to = "AUDIO";break;
		case 20: to = "AUDIO1";break;
		case 21: to = "AUDIO2";break;
		case 22: to = "AUDIO3";break;
		case 23: to = "AUDIO4";break;
		case 24: to = "NET";break;
		case 25: to = "Rhapsody";break;
		case 26: to = "SIRIUS IR";break;
		case 27: to = "Pandora";break;
		case 28: to = "SERVER";break;
		case 29: to = "NET RADIO";break;
		case 30: to = "USB";break;
		case 31: to = "iPod (USB)";break;
		case 32: to = "AirPlay";break;
    }
    return to;
}
...
Yamaha.prototype.setVolumeTo = function(to, zone) {
	//controle de la decimale car limité à 0 ou 5 et rien d'autre
		var toStr = to.toString();
		var toBase = toStr.substring(0,toStr.length-1);
		var toDec = parseInt(toStr.substring(toStr.length-1));
		if (toDec != 0 || toDec != 5){
            toDec = Math.round((toDec/5))*5*Math.sign(to); 
			to = parseInt(toBase+"0")+toDec;
		}	
	// limitation des volumes haut et bas _ add by TS
		if (to > this.maxDB) {to=this.maxDB;} 
		if (to < this.minDB) {to=this.minDB;}
...
Yamaha.prototype.setInputTo = function(to, zone) {
	...
	var command = '<YAMAHA_AV cmd="PUT"><' + getZone(zone) + '><Input><Input_Sel>' + getInput(to) + '</Input_Sel></Input></' + getZone(zone) + '></YAMAHA_AV>'; /* TS modification*/
    return this.SendXMLToReceiver(command);
};

Yamaha.prototype.setMainSceneTo = function(to) { /*TS addon */
    return this.setSceneTo(to, "Main_Zone");
};

Yamaha.prototype.setSceneTo = function(to, zone) { /*TS addon */
    var command = '<YAMAHA_AV cmd="PUT"><' + getZone(zone) + '><Scene><Scene_Load>Scene ' + to + '</Scene_Load></Scene></' + getZone(zone) + '></YAMAHA_AV>';
    return this.SendXMLToReceiver(command);
};
...
Yamaha.prototype.setPureDirect = function(on) {
    ...
    return this.sendPutCommand('<Sound_Video><Pure_Direct><Mode>' + ((on==1) ? 'On' : 'Off') + '</Mode></Pure_Direct></Sound_Video>'); /*TS modification*/
};
...
Yamaha.prototype.sleep = function(val, zone) {
    ...
    return this.sendPutCommand('<Power_Control><Sleep>' + val + '</Sleep></Power_Control>', getZone(zone));
};
...
Yamaha.prototype.selectTunerFrequency = function(band, frequency) { /* TS modification */
	band = (band == 1 ) ? "FM" : "AM" ;
    var unit = (band == "FM" ) ? "MHz" : "KHz",
        exp = (band == "FM") ? 2 : 0,
        command = '<YAMAHA_AV cmd="PUT"><Tuner><Play_Control><Tuning><Band>' + band + '</Band><Freq><' + band + '><Val>' + frequency + '</Val><Exp>' + exp + '</Exp><Unit>' + unit + '</Unit></' + band + '></Freq></Tuning></Play_Control></Tuner></YAMAHA_AV>';
...
Yamaha.prototype.selectFMmode = function(mode) { /* TS addons */
	mode = (mode == 1 ) ? "Auto" : "Mono" ;
    var command = '<YAMAHA_AV cmd="PUT"><Tuner><Play_Control><FM_Mode>'+mode+'</FM_Mode></Play_Control></Tuner></YAMAHA_AV>';
    return this.SendXMLToReceiver(command);
};
